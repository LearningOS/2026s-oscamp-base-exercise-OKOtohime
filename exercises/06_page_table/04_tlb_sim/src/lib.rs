...
impl Tlb {
    ...
    pub fn lookup(&mut self, vpn: u64, asid: u16) -> Option<u64> {
        ...
        self.entries
            .iter()
            .find(|entry| entry.valid == true && entry.vpn == vpn && entry.asid == asid)
            .and_then(|entry| {
                self.stats.hits += 1;
                Some(entry.ppn)
            })
...
impl Mmu {
    ...
    pub fn translate(&mut self, vpn: u64) -> Option<u64> {
        ...
        if let Some(ppn) = self
            .page_table
            .iter()
            .find(|(asid, _)| *asid == self.current_asid)
        {
            self.tlb
                .insert(vpn, ppn.1.ppn, self.current_asid, ppn.1.flags);
            return Some(ppn.1.ppn);
        }
        None
    }
}
