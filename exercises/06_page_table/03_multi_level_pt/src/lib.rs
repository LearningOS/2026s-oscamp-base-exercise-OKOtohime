...
impl Sv39PageTable {
            ...
            match level {
                1 | 2 => {
                    ...
                    if pte & PTE_V == 0 {
                        ...
                        current_ppn = (pte >> PPN_SHIFT) as u64;
                    }
                }
                0 => {
    ...
    pub fn translate(&self, va: u64) -> TranslateResult {
        ...
        let mut current_ppn = self.root_ppn; // 根页表物理内存地址
        for level in [2, 1, 0] {
            let vpn = Sv39PageTable::extract_vpn(va, level); // 页表项地址
            let node = match self.nodes.get(&current_ppn) {
                Some(n) => n,
                None => return TranslateResult::PageFault,
            };
            let pte = node.entries[vpn]; // 页表项
            if level > 0 && pte & PTE_V == 0 {
