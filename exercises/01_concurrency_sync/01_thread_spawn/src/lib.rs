...
//!   rayon: computation-intensive tokio: io-intensive
...
pub fn double_in_thread(numbers: Vec<i32>) -> Vec<i32> {
    ...
    let handle = thread::spawn(move || numbers.iter().map(|x| *x * 2).collect());
    handle.join().unwrap()
}
...
pub fn parallel_sum(a: Vec<i32>, b: Vec<i32>) -> (i32, i32) {
    ...
    thread::scope(|s| {
        let asum = s.spawn(|| a.iter().sum());
        let bsum = s.spawn(|| b.iter().sum());
        (asum.join().unwrap(), bsum.join().unwrap())
    })
}
